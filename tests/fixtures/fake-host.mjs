 ...
 * its own pid, so a test can tell which boot printed it. With FAKE_HOST_FORK=1
 * it also hands its listening socket to a child with the same command line, as
 * a forked worker would. It refuses the live host ports and exits by itself
 * after two minutes, so a crashed test cannot leave it behind.
 */
