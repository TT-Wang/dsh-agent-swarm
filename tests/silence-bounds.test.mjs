...
test('R16-D1: a pass wedged on live work is named inside its declared live-work bound, and both subjects are named', async t => {
  ...
  assert.match(notice.content, /The naming was held to its 240ms live-work bound by work that is preserved untouched/, 'the notice says the live work held the naming')
  assert.doesNotMatch(notice.content, /releas/i, 'the notice describes no release: the pass keeps the mission until its await settles')
...
test('R16-D3 pair: a control-path kick neither supersedes nor duplicates a wedged body, and the watchdog names it', async t => {
  ...
  assert.match(notice.content, /This pass keeps the mission's scheduling until the call it awaits returns at its own bound; the next pass then resumes from lease recovery\./,
    'the notice says what now happens: the named pass keeps the mission until its await settles, then the next pass resumes')
  assert.doesNotMatch(notice.content, /releas/i, 'and describes no release of the scheduling guard')
  f.runtime.scheduling.checkSchedulingPasses()
