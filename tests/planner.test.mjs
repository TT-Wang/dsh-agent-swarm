...
async function fixture(t, options = {}) {
  ...
  const runtime = new SwarmRuntime({ statePath: join(root, '.git', 'swarm.sqlite'), tickMs: 60000, leaseMs: 60000, maxMessageChars: 16000, maxEvents: 100, maxTasksPerMember: 3 }, { bind() {}, async dispose() {} })
  await runtime.start()
  ...
  await fiber
  t.after(async () => { await fiber.dispose(); pending.resolve(); await runtime.dispose(); await ctx.fiber.dispose(); await rm(root, { recursive: true, force: true }) })
...
test('dirty repository is preserved and rejected before model followup or durable request', async t => {
  const f = await fixture(t)
  await writeFile(join(f.root, 'user-work.txt'), 'keep this')
  await assert.rejects(f.execute('/agent-swarm make change'), /未提交/)
