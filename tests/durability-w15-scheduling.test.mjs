...
class QuietWorkers extends FakeWorkers {
  stopGate
...
async function fixture(t, config = {}) {
  const { dir: directory, runtime, workers, budget } = await makeRuntime(t, { workers: new QuietWorkers(),
    config: { maxMessageChars: 10000, maxEvents: 500, maxTasksPerMember: 100, checkTimeoutMs: undefined, ...config },
...
test('W15: a live handoff is neither reported stalled nor cancelled by completion', async t => {
  ...
  gate.resolve()
  const ready = await eventually(() => f.current(live.id).status === 'pending' ? f.current(live.id) : undefined, 'the handoff completes', 2500)
