...
const CENSUS = [
  ...
  ["src/planner.ts",1,"Set","const inFlight = new Set<string>()","outside","","outside the runtime decision path: the native planner adapter's bounded delivery-attempt registry; the durable planningDispatchPending/recoveryNoticePending flags and planning epoch select replayable work, while this set only prevents overlapping attempts for the same request generation"],
  ...
  ["src/runtime.ts",17,"Set","const seen = new Set<string>()","local","","function-local: created and discarded inside one synchronous call, so it cannot gate a later call"],
  ["src/runtime.ts",18,"Set","dependencyIdentities(missionId: string, dependencyId: string, tasks?: Task[]): Set<string> { return new Set(this.lineage(missionId, dependencyId, tasks).map(task => task.id)) }","local","","function-local: created and discarded inside one synchronous call, so it cannot gate a later call"],
  ["src/runtime.ts",19,"Set","const seen = new Set<string>([task.id])","local","","function-local: created and discarded inside one synchronous call, so it cannot gate a later call"],
  ["src/runtime.ts",20,"Map","const byId = new Map(tasks.map(task => [task.id, task]))","local","","function-local: created and discarded inside one synchronous call, so it cannot gate a later call"],
  ["src/runtime.ts",21,"Map","const chains = new Map<string, string[]>()","local","","function-local: created and discarded inside one synchronous call, so it cannot gate a later call"],
  ["src/runtime.ts",22,"Set","const candidate = visit(task.id, new Set())","local","","function-local: created and discarded inside one synchronous call, so it cannot gate a later call"],
  ["src/runtime.ts",23,"Set","const member: Member = { id: memberId, missionId, name, role: input.role, model: input.model, provider: input.provider, reasoningEffort: input.reasoningEffort, maxOutputTokens: input.maxOutputTokens, sessionId: id('swarm-session'), workspace, phase: 'active', status: deriveMemberStatus('active', false), subscriptions: input.subscriptions === undefined ? [] : [...new Set(input.subscriptions)] }","local","","function-local: created and discarded inside one synchronous call, so it cannot gate a later call"],
  ["src/runtime.ts",24,"Set","knownContents: new Set(this.store.list('tasks', missionId).map(task => task.id)),","local","","function-local: created and discarded inside one synchronous call, so it cannot gate a later call"],
  ["src/runtime.ts",25,"Set","const dependencies = [...new Set(normalizeReviewDependencies(input.kind, input.reviewOf, input.dependencies))]","local","","function-local: created and discarded inside one synchronous call, so it cannot gate a later call"],
  ...
  ["src/runtime.ts",33,"Set","const live = new Set(this.store.list('members', missionId).filter(member => memberPhaseOf(member) !== 'stopped').map(member => member.id))","local","","function-local: created and discarded inside one synchronous call, so it cannot gate a later call"],
  ["src/runtime.ts",34,"Set","const released = new Set<string>()","local","","function-local: created and discarded inside one synchronous call, so it cannot gate a later call"],
  ["src/runtime.ts",35,"Set","member.subscriptions = [...new Set(topics)]","local","","function-local: created and discarded inside one synchronous call, so it cannot gate a later call"],
  ["src/runtime.ts",36,"Map","const byKey = new Map(plan.tasks.map(task => [task.key, task]))","local","","function-local: created and discarded inside one synchronous call, so it cannot gate a later call"],
  ["src/runtime.ts",37,"Set","const pending = [...(byKey.get(key)?.dependencies ?? [])], visited = new Set<string>()","local","","function-local: created and discarded inside one synchronous call, so it cannot gate a later call"],
  ...
  ["src/store.ts",4,"Set","return new Set(rows.map(row => String(row.owner_id)))","local","","R17-G7 function-local: the owner ids of the running attempts in one mission, read from the durable tasks table and discarded inside the same synchronous hydration, so it cannot gate a later call"],
  ["src/trace.ts",1,"Set","const reclaimed = new Set<string>()","local","","function-local: created and discarded inside one sweep call, so it cannot gate a later call; the selection it drives is re-derived from the files' mtimes and the declared bound on every later sweep"],
  ["src/trace.ts",2,"WeakMap","const hostTelemetryLinks = new WeakMap<object, HostTelemetryLink>()","outside","","outside the runtime decision path: the runtime-to-host-sink link registry for the telemetry transport; a missing link can drop one reporting record, never a durable row or a mission decision (the span row is appended before the sink is consulted)"],
  ["src/trace.ts",3,"Map","private readonly indexes = new Map<string, SpanIndex>()","outside","","outside the runtime decision path: the trace read model (TraceIndex): an owner-UI projection cache no runtime decision reads (enumerated, no label claimed)"],
  ["src/trace.ts",4,"Map","private readonly unscoped = new Map<string, number>()","outside","","outside the runtime decision path: the trace read model (TraceIndex): an owner-UI projection cache no runtime decision reads (enumerated, no label claimed)"],
  ["src/trace.ts",5,"Map","const index: SpanIndex = { byTask: new Map(), byAttempt: new Map(), all: [] }","outside","","outside the runtime decision path: the trace read model (TraceIndex): an owner-UI projection cache no runtime decision reads (enumerated, no label claimed)"],
  ...
  // call, so it cannot gate a later one: the pool order and the durable rows are
  // the state, and losing the Set merely recomputes the same next name.
  ["src/types.ts",1,"Set","const taken = new Set(used)","local","","function-local: created and discarded inside one synchronous call, so it cannot gate a later call"],
...
const GATE_TESTS = {
  ...
  'src/notices.ts:9': async t => {
    const f = await setup({ config: { tickMs: 10 } })
    try {
      f.runtime.notices.pendingTransitions.add('probe-mission')   // the loss must be exercised on a non-empty collection
      assert.ok(f.runtime.notices.pendingTransitions.size > 0)
      f.runtime.notices.pendingTransitions.clear()   // the loss
      // The commit itself is the gate: a durable notice still lands after the
      // publication queue was lost, because the write and its fact are committed.
      f.runtime.commit(f.mission.id, () => f.runtime.notify(f.mission.id, 'probe after a lost queue', [`mission:${f.mission.id}`], { trigger: 'probe/lost-queue', reason: 'probe' }))
      const queued = f.runtime.store.list('deliveries', f.mission.id).filter(delivery => typeof delivery.notice?.dedupKey === 'string' && delivery.notice.dedupKey.startsWith('probe/lost-queue:'))
      assert.equal(queued.length, 1, 'the durable outbox is unaffected by the lost publication queue')
    } finally { await f.cleanup() }
  },
  'src/notices.ts:10': async t => {
    const f = await setup({ config: { tickMs: 10 } })
    try {
      f.runtime.notices.wedgedReleases.add(f.mission.id)   // the loss must be exercised on a non-empty collection
      assert.ok(f.runtime.notices.wedgedReleases.size > 0)
      f.runtime.notices.wedgedReleases.clear()   // the loss
      const task = f.propose({ title: 'Wedged marker lost' })
      assert.ok(task?.id, 'the durable write is unaffected by the lost wedge marker')
      // The release itself is durable; the next transition still publishes.
