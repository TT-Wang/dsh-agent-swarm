...
On the supported `0.1.5-rc.1` and `0.1.6-alpha.2` hosts, Agent Swarm lives in the native right sidebar beside Files (on 0.1.6 it is a capsule on the sidebar's Start page). Earlier supported hosts use Better Sidebar when available, or the plugin's dock. The UI supports English, Chinese, light and dark themes.
