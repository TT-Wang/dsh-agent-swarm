...
export function newPlan(workspace: string, budget: Budget): PlanInput {
  ...
  return {
    ...
    tasks: [
      { key: 'deliver', workstreamKey: 'delivery', title: 'Deliver the mission', objective: 'Implement the mission objective, run relevant checks, publish evidence, and submit an immutable artifact.', kind: 'integration', scope: ['**'], acceptance: [...acceptance], checks: ['git diff --check HEAD^ HEAD'], assigneeKey: 'builder', dependencies: [] },
    ...
    <fieldset disabled={Boolean(busy) || !editable}>
      ...
      <section className="sw-section"><div className="sw-row"><h3>{t('Tasks')}</h3><button type="button" onClick={() => update({ tasks: [...input.tasks, { key: freshKey('task'), workstreamKey: input.workstreams[0]?.key ?? '', title: '', objective: '', kind: 'research', scope: [...input.scope], acceptance: [...input.acceptance], dependencies: [] }] })}>{t('Add task')}</button></div>
        {input.tasks.map(task => <details className="sw-edit-item" key={task.key} open><summary>{task.title || task.key} · {task.kind}</summary>
          ...
          {task.kind === 'verification' ? <p className="sw-small">{t('Verification runs the selected source task’s checks on its immutable artifact.')}</p> : ['implementation', 'integration'].includes(task.kind) && <p className="sw-small">{t('Replace the default formatting check with checks that prove your acceptance criteria.')}</p>}
        </details>)}
      </section>
    </fieldset>
