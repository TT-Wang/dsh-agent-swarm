...
Choose the smallest useful team with independent review. Omit name for host-assigned identities; put responsibilities in role. Split independently verifiable outcomes when parallel work outweighs coordination/review/integration cost. Dependencies name required artifacts; share a small interface contract. Automatic assigneeKey is a preference: idle eligible members can borrow untouched tasks. Use assignmentMode=pinned for a required member/model. Every deliverable needs independent verification: reviewOf pins the source and starts at submission. One implementation needs one review; multiple implementation branches also need an integration depending on all results and its review. Cover each mission acceptance string verbatim in at least one deliverable task; not every task needs every criterion. Include named output paths within each writing task scope. Spread reviews across independent available members; inspect attemptOwner/assigneeId for the actual executor, plannedAssigneeId is only a preference. Host checks run on immutable artifacts.
${TASK_PLANNING_RULES}
...
export function registerTools(ctx: Context, runtime: SwarmRuntime, defaultBudget: Budget, grants?: WorkspaceGrantSnapshot): void {
  ...
  const taskChangesSchema: JsonSchemaNode = { type: 'object', additionalProperties: false, properties: { ...taskBudgetSchema.properties, scope: scopeSchema, dependencies: { ...strings, description: 'Complete replacement list, not additions; omitted leaves dependencies unchanged. The result reports added and removed dependencies.' }, checks: strings, assigneeId: { type: 'string', description: 'Member id; an empty string releases the binding after confirmed stop.' } } }
