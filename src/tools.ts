...
Choose the smallest useful team with independent review. Omit name for host-assigned identities; put responsibilities in role. Split independently verifiable outcomes when parallel work outweighs coordination/review/integration cost. Dependencies name required artifacts; share a small interface contract. Automatic assigneeKey is a preference: idle eligible members can borrow untouched tasks. Use assignmentMode=pinned for a required member/model. Every deliverable needs independent verification: reviewOf pins the source and starts at submission. One implementation needs one review; multiple implementation branches also need an integration depending on all results and its review. Cover each mission acceptance string verbatim in at least one deliverable task; not every task needs every criterion. Declare files a task writes in outputs per schema; [] if it only reads/deletes. Spread reviews across independent available members; inspect attemptOwner/assigneeId for the actual executor, plannedAssigneeId is only a preference. Host checks run on immutable artifacts.
${TASK_PLANNING_RULES}
