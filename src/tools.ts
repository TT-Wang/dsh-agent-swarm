...
Choose the smallest useful team with independent review. Omit name for host-assigned identities; put responsibilities in role. Split independently verifiable outcomes when parallel work outweighs coordination/review/integration cost. Dependencies name required artifacts; share a small interface contract. Automatic assigneeKey is a preference: idle eligible members can borrow untouched tasks. Use assignmentMode=pinned for a required member/model. Every deliverable needs independent verification: reviewOf pins the source and starts at submission. One implementation needs one review; multiple implementation branches also need an integration depending on all results and its review. Cover each mission acceptance string verbatim in at least one deliverable task; not every task needs every criterion. List every file a task must produce in its outputs, each within that task's scope; an analysis-only task declares an empty array. Spread reviews across independent available members; inspect attemptOwner/assigneeId for the actual executor, plannedAssigneeId is only a preference. Host checks run on immutable artifacts.
${TASK_PLANNING_RULES}
...
export function registerTools(ctx: Context, runtime: SwarmRuntime, defaultBudget: Budget, grants?: WorkspaceGrantSnapshot): void {
  ...
  register('swarm_propose', 'Propose and admit a task within mission scope and budget. research for audits and synthesis; implementation/integration need real checks; verification names reviewOf. Rejected implementations are repaired with replaces and unchanged acceptance. Raise an existing task\'s allocation with swarm_budget(taskId, taskBudget, reason); amend its unsubmitted scope, dependencies, checks or assignee with swarm_control(taskId, action: amend, changes, reason). Correct admission field errors and retry the proposal.',
    { ...mission, workstreamId: string, title: string, objective: string, kind: kindSchema, dependencies: dependenciesSchema, scope: scopeSchema, acceptance: strings, outputs: outputsSchema, checks: checksSchema, priority: integer, maxRecoveryAttempts: integer, maxSteps: taskCeilingSchema.maxSteps, maxFindings: taskCeilingSchema.maxFindings, checkTimeoutMs: integer, experiment: { type: 'boolean' }, assigneeId: string, assignmentMode: assignmentModeSchema, reviewOf: reviewSchema, replaces: strings },
    ['missionId', 'workstreamId', 'title', 'objective', 'kind', 'scope', 'acceptance', 'outputs'],
