...
**返工保留原来的任务目标。** 任务被独立复核驳回后，主 agent 可以在原任务上返工：`swarm_control` 的 `action: "resume"` 会把被驳回的任务重新交给原作者（作者保持固定，从被驳回的 commit 继续修改），并以同一位复核人重新打开作出驳回的复核，因此重新提交后立即复核，不会新增复核任务。每个任务最多返工 `maxRework` 次（默认 2 次，可在 `swarm_control` 的 `changes` 中提高）；超出后，用 `replaces` 提出的替代任务继承原验收标准，下游依赖跟随已验收的修复，替代任务被验收时会退役它所替代的整条链。被驳回的 commit、驳回理由和该轮的声明作为归档历史保留在任务及其复核上：不能再次质疑，也不会阻止完成。主 agent 还可以在原任务上修订执行额度和未提交工作的计划，按新版本修复启动失败的保存计划，并在不改变工件的情况下为已提交工作追加检查。
...
在两个受支持宿主上，Agent Swarm 都位于原生右侧栏，默认折叠。发送 `/agent-swarm` 加自然语言目标后会自动展开；也可以随时点击左侧导航底部的 Agent Swarm 按钮重新打开。右侧栏属于单个会话：打开某个成员的会话后，用同一个按钮打开它的只读协作视图。界面支持中英文和深浅主题。
...
- **只有 0.1.7 会强制执行版本约束。** 在已检查的版本中，只有 0.1.7-rc.1 会把插件固定的 `@deepseek-ai/dsh-*` peer 版本范围与正在运行的版本比较，不匹配时停用插件。0.1.5-rc.3 没有这项检查，插件在其他 0.1.5 或 0.1.6 版本上加载时不会被拒绝：在这些宿主上，“只支持两个版本”只是文档声明，并无强制。profile bundle 中的 `requires.harness` 在两个宿主上都只是声明。
- **可审查的证据。** 独立复核与宿主检查让验收有据可查，但无法保证所选检查覆盖所有要求。验证默认复制已有依赖及常见虚拟环境解释器，仍可能依赖宿主系统库。不支持的外部依赖链接需要改为自包含安装或由宿主显式授权；这不是全新的 CI 环境。
- **原任务返工。** 只有被自身独立复核驳回的任务可以返工；被驳回的实验任务，或提交后被判定失效的工作，仍需替代任务。作者的步数与恢复次数在多次返工间累计，额度用尽时 resume 会以 `task_budget_exhausted` 拒绝，需先提高额度；重新打开的复核每一轮都从全新额度开始。重新提交必须改变 commit 或带来新的宿主可验证声明，否则以 `[rework_unchanged]` 拒绝。被重新打开的复核所绑定的成员永远不能接手返工；若它是唯一仍在线的成员，任务会等待主 agent 增加成员或改派该复核。
- **宿主补充的复核。** 计划中没有配对复核的交付任务，会在启动、保存草稿以及列出旧版本保存的草稿时获得一个未分配的独立复核，并计入 `maxTasks`。只有当某个未停止的成员可以独立承担时，该复核才算有效的复核路径；因此若计划中唯一在线的成员都是交付物作者，提交后会以 `[review_path_missing]` 通知 owner。
...
- **可观察、有边界的恢复。** 实时界面展示操作和事件，不提供逐 token 输出或保证准确的完成时间。恢复机制不承诺外部副作用恰好执行一次、任意磁盘故障恢复或整个文件系统上的原子应用。卡在慢速 worker 调用中的调度轮次会一直占住该任务，直到该调用在自身时限内返回；超过界限时会通知 owner。
