...
在受支持的 `0.1.5-rc.1` 与 `0.1.6-alpha.2` 宿主上，Agent Swarm 位于 Files 旁的原生右侧栏（0.1.6 上是右侧栏 Start 页里的一张卡片）。较早的受支持版本优先使用 Better Sidebar，否则使用插件自身的停靠面板。界面支持中英文和深浅主题。
...
- **文件交付物。** 成员通过 `swarm_submit.deliverables` 列出准确的相对文件路径；即使被 Git 忽略，宿主也能只捕获这些明确文件，不修改源检出的忽略规则。提交回显绑定 commit 的 blob ID 和字节数，复核以固定源 commit 为准，主机检查在该版本的新检出中执行，作者后续覆写不会改变被复核的报告。同源复核改派会保留已保存草稿。任务文字中推断出的遗漏忽略路径会列入 `artifact.uncapturedPaths` 提示；未声明的任意忽略文件不会自动捕获。
